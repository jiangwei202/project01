	...
	<view>
		...
		<view class="order-status">
			...
			<block v-if="orderStatus == 10">
				...
				<view class="od-stt">
					请在 
					<counter style="font-size: 24rpx;" :second="order._exp_time" @counterOver="onCounterOver"></counter>
					内完成支付，超时订单取消
				</view>
				
				<view class="od-op" @click="payment">
					<button class="btnp" @click="dopay">立即支付({{order.price}})</button>
				</view>
			</block>
			<!-- 进行中 -->
			<block v-if="orderStatus == 20">
				<block v-if="order.service_state == 0">
					<view><text class="od-st">正在为您按排服务专员...</text></view>
					<view class="od-stt">请保持手机畅通，稍后有服务专员与您联系</view>
				</block>
